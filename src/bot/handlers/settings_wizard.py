...
async def start_edit_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancelar", callback_data=f"manage_bot_{bot_id}")]])
...
async def receive_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.photo or msg.video or msg.document:
        await msg.reply_text("❌ <b>Erro:</b> O perfil do Telegram aceita apenas texto.", parse_mode="HTML")
        return WAITING_DESCRIPTION
    ...
    if not success:
        await msg.reply_text("⚠️ Salvei no banco, mas o Telegram rejeitou atualizar o perfil (texto muito longo?).")
    ...
    text = TextUtils.pad_message(f"<b>⚙️ Gerenciando: {saved_bot.name}</b>\nO que deseja fazer?")
    await ChatManager.render_view(update, context, text, bot_management_keyboard(saved_bot))
...
async def receive_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text("✅ <b>Boas-vindas Atualizada!</b>\nVeja o preview:", parse_mode="HTML")

    if media_type == 'photo':
        await context.bot.send_photo(chat_id=msg.chat_id, photo=media_id, caption=welcome_text, parse_mode="HTML")
    elif media_type == 'video':
        await context.bot.send_video(chat_id=msg.chat_id, video=media_id, caption=welcome_text, parse_mode="HTML")
    else:
        await context.bot.send_message(chat_id=msg.chat_id, text=welcome_text, parse_mode="HTML")
...
async def cancel_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(Bot).filter(Bot.id == bot_id))
        bot = result.scalars().first()
        text = TextUtils.pad_message(f"<b>⚙️ Gerenciando: {bot.name}</b>\nOperação cancelada.")
...
settings_wizard_handler = ConversationHandler(
    ...
    states={
        ...
        WAITING_WELCOME: [
            MessageHandler((filters.TEXT | filters.PHOTO | filters.VIDEO) & ~filters.COMMAND, receive_welcome)
        ]
    },
    fallbacks=[
