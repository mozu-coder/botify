...
async def view_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📜 Extrato Detalhado", callback_data="wallet_extract")],
        [InlineKeyboardButton("💸 Solicitar Saque", callback_data="wallet_withdraw")],
...
async def view_extract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not transactions:
        ...
        for t in transactions:
            if "Taxa de Serviço" in t.description: continue
    ...
    text = TextUtils.pad_message("\n".join(msg_lines))
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Voltar", callback_data="wallet_view")]])
    await ChatManager.render_view(update, context, text, kb)
...
async def start_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if balance < settings.MIN_WITHDRAWAL:
        await UI.show_toast(update, f"Mínimo para saque: {TextUtils.currency(settings.MIN_WITHDRAWAL)}", alert=True)
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancelar", callback_data="wallet_view")]])
    await ChatManager.render_view(update, context, text, kb)
...
async def receive_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        amount_gross = float(text_val) # O valor que ele digitou (Total)
    except ValueError:
    ...
    # Calcula quanto ele vai receber DE FATO
    ...
    context.user_data['withdraw_gross'] = amount_gross # Guardamos o Bruto
...
async def receive_pix(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    amount_gross = context.user_data['withdraw_gross'] # Pega o Bruto
    user_id = update.effective_user.id
    user_name = update.effective_user.full_name
    username = update.effective_user.username or "SemUser"
    
    try:
        # Passamos o valor Bruto. O service faz a mágica.
        withdrawal = await FinanceService.request_withdrawal(user_id, amount_gross, pix_key)
        current_balance = await FinanceService.get_balance(user_id)
        
        # O Admin vê o valor LÍQUIDO que tem que pagar (amount_requested)
        admin_text = (
        ...
        admin_kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ Confirmar Pagamento", callback_data=f"admin_pay_{withdrawal.id}"),
                InlineKeyboardButton("❌ Rejeitar/Estornar", callback_data=f"admin_reject_{withdrawal.id}")
            ]
        ])
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Ir para Carteira", callback_data="wallet_view")]])
        await ChatManager.render_view(update, context, text, kb)
        return ConversationHandler.END
