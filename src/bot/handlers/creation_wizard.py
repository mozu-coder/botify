...
async def start_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inicia o wizard de criação de bot solicitando o token."""
...
async def receive_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Valida o token recebido e solicita vinculação do grupo."""
        ...
        [
            ...
            [
                InlineKeyboardButton(
                    "🔄 Verificar Grupo", callback_data="check_group_connection"
                )
            ],
            [InlineKeyboardButton("� Cancelar", callback_data="cancel_wizard")],
        ]
...
async def check_group_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await ChatManager.render_view(
            update, context, success_text, main_menu_keyboard()
        )
