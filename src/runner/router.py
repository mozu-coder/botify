...
async def process_update_task(token: str, update_data: dict):
    """
    Processa updates do Telegram em background para os bots filhos.
    """
...
async def runner_webhook(token: str, request: Request, background_tasks: BackgroundTasks):
...
async def payment_webhook(request: Request):
    """
    Processa a confirmação de pagamento da GGPIX.
    """
    ...
    if data.get("type") == "PIX_IN" and data.get("status") == "COMPLETE":
        ...
        # --- CORREÇÃO: Ignora se não tiver ID externo ---
        if not external_id:
        ...
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(select(Transaction).filter(Transaction.external_id == external_id))
            transaction = result.scalars().first()
            
            # Se não achou ou se já foi processada (valor > 0), ignora
            if not transaction:
            ...
            # Processa liberação do Plano
            # Garante que external_id é string antes do split
            ...
            if len(parts) >= 3:
                ...
                bot_res = await session.execute(select(Bot).filter(Bot.id == transaction.bot_id))
                bot = bot_res.scalars().first()
                
                plan_res = await session.execute(select(Plan).filter(Plan.id == plan_id))
